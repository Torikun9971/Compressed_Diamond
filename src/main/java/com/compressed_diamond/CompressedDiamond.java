    ...
    private static class SetTooltips {
        ...
        private static void setTooltips(ItemTooltipEvent event) {
            if (event.getItemStack().is(Items.DIAMOND_BLOCK) && Screen.hasShiftDown()) {
                event.getToolTip().add(new TranslatableComponent("info.compressed_diamond.diamonds", new DecimalFormat("#,###").format(BigInteger.valueOf(9).multiply(BigInteger.valueOf(event.getItemStack().getCount())))));
            }
        }
    }
}
